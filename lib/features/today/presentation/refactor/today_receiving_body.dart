...
class TodayReceivingBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<TodayCubit, TodayState>(builder: (context, state) {
      ...
      child: SafeArea(
        ...
        child: Padding(
          ...
          child: Column(children: [
            Text('today_receiving_title'.tr(), style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 4),
            Text(MilkDateUtils.formatFullArabic(today), style: TextStyle(color: Colors.white.withValues(alpha: 0.85), fontSize: 13)),
            const SizedBox(height: 12),
            Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              _chip('received'.tr(), MilkDateUtils.toArabicNumerals(state.receivedList.length)),
              const SizedBox(width: 10),
              _chip('total_kg'.tr(), MilkDateUtils.toArabicNumeralsStr(state.totalToday.toStringAsFixed(1))),
            ]),
          ]),
        ),
      ),
  ...
  Widget _chip(String label, String value) => Container(
    ...
    child: Text('$value $label', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
  );
  ...
  Widget _buildLockedBanner() => Container(
    ...
    child: Row(children: [
      ...
      Expanded(child: Text('week_locked_banner'.tr(), style: const TextStyle(color: AppColors.edited, fontSize: 12))),
    ]),
  );

  Widget _buildSearchRow(BuildContext context, TodayState state) {
    final cubit = context.read<TodayCubit>();
    final villages = state.suppliers.map((s) => s.village).whereType<String>().toSet().toList();
    return Padding(
      ...
      child: Row(children: [
        ...
        if (villages.isNotEmpty) ...[
          ...
          PopupMenuButton<String?>(
            icon: Icon(Icons.filter_list, color: state.villageFilter != null ? AppColors.primary : AppColors.textSecondary),
  ...
  Widget _buildTabs(TodayState s) => TabBar(tabs: [
    Tab(text: '${'tab_pending'.tr()} (${MilkDateUtils.toArabicNumerals(s.pendingList.length)})'),
    Tab(text: '${'tab_received'.tr()} (${MilkDateUtils.toArabicNumerals(s.receivedList.length)})'),
    Tab(text: '${'tab_no_milk'.tr()} (${MilkDateUtils.toArabicNumerals(s.noMilkList.length)})'),
  ]);
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        return PendingCard(
          ...
          onNoMilk: isPending && !state.isWeekLocked ? () => showNoMilkDialog(ctx, supplier) : null,
        );
      },
    );
  }
