...
class PendingCard extends StatelessWidget {
  ...
  Widget _recordedBadge() {
    ...
    return Container(
      ...
      child: Text(
        '${MilkDateUtils.toArabicNumeralsStr(kg)} ${'unit_kg'.tr()}',
        style: const TextStyle(
          color: AppColors.primary,
          fontWeight: FontWeight.bold,
          fontSize: 13,
  ...
  Widget _actions() {
    if (isLocked)
      return const Icon(Icons.lock, size: 18, color: AppColors.textHint);
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        _actionBtn('enter_weight'.tr(), AppColors.primary, onWeightEntry),
        const SizedBox(width: 6),
        _actionBtn('no_milk_label'.tr(), AppColors.noMilk, onNoMilk),
      ],
    );
  }
